   ...
   public static void registerCommands(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess, CommandManager.RegistrationEnvironment registrationEnvironment){
      dispatcher.register(literal("nations").requires(source -> source.hasPermissionLevel(2))
            .then(literal("debug")
                  ...
                  .then(literal("surveyWorld").requires(source -> source.hasPermissionLevel(2)).executes(NationsCommands::surveyWholeWorld))
                  .then(literal("cacheCheck").requires(source -> source.hasPermissionLevel(2)).executes(NationsCommands::cacheCheck))
                  .then(literal("purgeCache").requires(source -> source.hasPermissionLevel(2)).executes(NationsCommands::purgeCache))
            )
            ...
            .then(literal("unclaimChunks").requires(source -> source.hasPermissionLevel(2))
                  .then(argument("nation_id", word()).suggests(NationsCommands::getNationSuggestions)
                        .executes(context -> NationsCommands.unclaimChunks(context, getString(context, "nation_id")))))
            .then(literal("relocateMonument").requires(source -> source.hasPermissionLevel(2))
                  .then(argument("nation_id", word()).suggests(NationsCommands::getNationSuggestions)
                        .executes(context -> NationsCommands.resettle(context, getString(context, "nation_id"), ChunkSectionPos.from(context.getSource().getPosition())))
                        .then(argument("x", integer())
                              .then(argument("z", integer())
                                    .executes(context -> NationsCommands.resettle(context, getString(context, "nation_id"), ChunkSectionPos.from(getInteger(context,"x"),0,getInteger(context,"z"))))))))
            ...
            .then(literal("unclaimCap").requires(source -> source.hasPermissionLevel(2))
                  .executes(context -> NationsCommands.unclaimCapturePoint(context, ChunkSectionPos.from(context.getSource().getPosition())))
                  .then(argument("x", integer())
                        .then(argument("z", integer())
                              .executes(context -> NationsCommands.unclaimCapturePoint(context, ChunkSectionPos.from(getInteger(context,"x"),0,getInteger(context,"z")))))))
      ...
      dispatcher.register(literal("nation")
            ...
            .then(literal("claim")
                  .executes(context -> NationsCommands.claimChunk(context, ChunkSectionPos.from(context.getSource().getPosition())))
                  .then(argument("x", integer())
                        .then(argument("z", integer())
                              .executes(context -> NationsCommands.claimChunk(context, ChunkSectionPos.from(getInteger(context,"x"),0,getInteger(context,"z")))))))
            ...
            .then(literal("info")
                  .executes(NationsCommands::nationInfo)
                  .then(argument("nation_id", word()).requires(source -> source.hasPermissionLevel(2)).suggests(NationsCommands::getNationSuggestions)
                        .executes(context -> NationsCommands.nationInfo(context, getString(context,"nation_id")))))
